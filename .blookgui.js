;(async () => {
    ...
    if (
    (console.log(
    '%c Blooket Cheats Plus %c\n	By DannyDan0167/CryptoDude3 on GitHub',
    'color: #0bc2cf; font-size: 3rem',
    'color: #8000ff; font-size: 1rem'
    ),
    console.log('%c	gui.js', 'color: #0bc2cf; font-size: 1rem'),
    console.log(
    '%c	Star the github repo!%c  //github.com/randomstuff69/blooketcheatsplus',
    ...
    t = (document.body.appendChild(e), (window.clearId = window.setInterval(() => {}, 0)));
    t--;
    ...
    link.setAttribute(
    'href',
    '//fonts.googleapis.com/css?family=Nunito:400,700|Titan+One|Creepster|Satisfy|Eczar:700'
    )
    ...
    .split('.')
    .reduce(
    (e, a, o, r) => ((e[a] = e[a] ? e[a] : {}), ++o == r.length && (e[a] = t), e[a]),
    this.data
    ),
    ...
    deleteItem(e) {
    return (
    e.split('.').reduce((e, t, a, o) => (++a == o.length && delete e[t], e[t]), this.data),
    localStorage.setItem('JODGUISettings', JSON.stringify(this.data)),
    this.data
    )
    },
    ...
    try {
    for (let d of ((c.data = JSON.parse(localStorage.getItem('JODGUISettings') || '{}')),
    [
    'backgroundColor',
    'cheatList',
    'contentBackground',
    ...
    id: 'backgroundImage',
    style: {
    backgroundImage: 'url(//ac.blooket.com/dashboard/65a43218fd1cabe52bdf1cda34613e9e.png)',
    display: 'block',
    height: '200%',
    position: 'absolute',
    width: '200%',
    ...
    innerText:
    (([{ctrl: e, shift: t, alt: a, key: o}, {ctrl: r, shift: i, alt: n, key: s}] = [
    c.data.hide || {ctrl: !0, key: 'e'},
    c.data.close || {ctrl: !0, key: 'x'},
    ]),
    ...
    update: (
    {ctrl: e, shift: t, alt: a, key: o} = {ctrl: !0, key: 'e'},
    {ctrl: r, shift: i, alt: n, key: s} = {ctrl: !0, key: 'x'}
    ) =>
    ...
    onclick: () =>
    window.open('//github.com/randomstuff69/blooketcheatsplus', '_blank').focus(),
    }),
    ($ = l(
    'div',
    {
    ...
    t != $ &&
    (e
    ? (t.style.display = t.style._display)
    : ((t.style._display = t.style.display), (t.style.display = 'none')))
    ...
    innerHTML:
    '<span style=\'text-shadow: 1px 1px rgb(0 0 0 / 40%); font-size: 0.8em;\'>Cheats<sup>v15.5</sup></span>',
    },
    l('a', {
    className: 'bigButton',
    style: {
    ...
    l(
    'div',
    {id: 'content', style: {position: 'absolute', inset: '27px 50px 50px 50px'}},
    (y = l('div', {
    className: 'tooltip',
    style: {
    position: 'absolute',
    ...
    innerHTML:
    ('string' == typeof t ? `<img style='height: 30px; margin-right: 5px' src='${t}'>` : t || '') +
    e,
    ...
    ? `<img style='height: 30px; margin-right: 5px' src='${imgSrc}'>`
    : imgSrc || '') + name,
    onclick: () => k(r.innerText, a, o),
    })
    ...
    let {name: r, description: i, type: n, inputs: s, enabled: c, run: d, element: p} = t[o]
    if (!p) {
    let u = l(
    'div',
    {
    ...
    l('div', {className: 'cheatName', innerHTML: r})
    )
    if (
    ((u.dataset.description = i),
    (u.onclick = function ({target: e, key: t}) {
    ;(e == u ||
    e.classList.contains('cheatName') ||
    ('Enter' == t && e.classList.contains('cheatInput'))) &&
    ((t = [...u.children].slice(1)),
    d.apply(
    this,
    t.map((e) =>
    'number' == e.type
    ...
    'toggle' == n &&
    (u.style.background = this.enabled ? 'var(--enabledButton)' : 'var(--disabledButton)'),
    C.alerts?.[0].addLog(
    `${'toggle' == n ? (this.enabled ? 'Enabled' : 'Disabled') : 'Ran'} <strong>${this.name}</strong>` +
    (s?.length
    ? ` with inputs: (${t.map((e) => ('SELECT' == e.nodeName ? e.selectedOptions[0].innerText : e.value)).join(', ')})`
    : ''),
    'toggle' == n ? (this.enabled ? 'var(--enabledButton)' : 'var(--disabledButton)') : null
    ))
    }.bind(t[o])),
    s?.length)
    )
    for (let h = 0; h < s.length; h++) {
    var {name: m, type: $, options: g, min: y, max: _, value: f} = s[h]
    let w
    try {
    ...
    ;(S.onclick = async () => {
    x ||
    ((S.value = 'Waiting for input...'),
    (x = !0),
    (S.data = await s[h].function((e) => (S.value = e + '...'))),
    (x = !1),
    (S.value = S.value.slice(0, -3)))
    }),
    u.appendChild(S)
    } else
    (g = document.createElement('input')).classList.add('cheatInput'),
    'number' == $ &&
    ((g.type = 'number'), (g.min = y), (g.max = _), (g.value = f || (null != y ? y : 0))),
    (g.placeholder = m),
    ...
    : ((this.enabled = !0),
      ...
      } = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
      .stateNode
      let r = e || o
      try {
      'typing' != r.qType
      ? ('feedback' === t || a
        ...
        : [...document.querySelectorAll('[class*=\'answerContainer\']')][
          r.answers
          .map((e, t) => (r.correctAnswers.includes(e) ? t : null))
          .filter((e) => null != e)[0]
          ]
        )?.click?.()
      : Object.values(
    ...
    : ((this.enabled = !0),
      ...
      (e.question || t.client.question).answers[o]
      )
      ? (a.style.backgroundColor = 'rgb(0, 207, 119)')
    ...
    this.enabled = !0
    let t = () => {
    e.props.liveGameController.setVal({path: `c/${e.props.client.name}/tat/Freeze`, val: 'freeze'})
    }
    ...
    if (
    document.querySelector('[class*=\'answerContainer\']') ||
    document.querySelector('[class*=\'typingAnswerWrapper\']')
    ) {
    let o = 0,
    r = 0
    ...
    s ? a.answers[0] : Math.random().toString(36).substring(2)
    )
    }
    } catch {}
    },
    ...
    type: 'options',
    options: () =>
    Array.from(document.querySelectorAll('[class*=\'packsWrapper\'] > div')).reduce(
    (e, t) => (
    t.querySelector('[class*=\'blookContainer\'] > img') ||
    e.push(t.querySelector('[class*=\'packImgContainer\'] > img').alt),
    e
    ),
    []
    ),
    ...
    i = Array.from(document.querySelectorAll('[class*=\'packsWrapper\'] > div')).reduce(
    (e, t) => (
    t.querySelector('[class*=\'blookContainer\'] > img') ||
    (e[t.querySelector('[class*=\'packImgContainer\'] > img').alt] = parseInt(
    ...
    let u = Date.now()
    ;(d += Date.now() - u),
    r.setState({canOpen: !0, currentPack: '', opening: a, doneOpening: a, openPack: a}),
    clearTimeout(r.canOpenTimeout)
    }
    await new Promise((e) => setTimeout(e)),
    alert(
    `(${Date.now() - d}ms) Results: ${Object.entries(c)
    .map(([e, t]) => `    ${e} ${t}`)
    .join(` `)}`
    )
    },
    ...
    let r = await fetch('//fb.blooket.com/c/firebase/join', {
    ...
    let n = i.database()
    await n.ref(`${e}/c/${t}`).set({b: a, bg: o}), C.alerts[0].addLog(`Bot ${t} joined!`)
    ...
    inputs: [{name: 'Blook (Case Sensitive)'}],
    run: (unlockedBlook) => {
    ...
    type: 'options',
    options: () =>
    [...document.querySelector('[class*=packsWrapper]')?.children]?.map(
    (e) => e.children[0].children[0].alt
    ),
    },
    ],
    run: async (packName) => {
    ...
    inputs: [{name: 'New Name', type: 'text'}],
    ...
    const res = await fetch('//fb.blooket.com/c/firebase/join', {
    body: JSON.stringify({id: reactHandler().props.client.hostId, name}),
    ...
    reactHandler().props.liveGameController.setVal({path: `c/${newname}`, val: olddata})
    }
    })
    ...
    inputs: [{name: 'Banner ID', type: 'text'}],
    run: function (e) {
    var t = document.createElement('iframe')
    ...
    type: 'options',
    async options() {
    let {webpack: e} = webpackJsonp.push([
    [],
    {
    1234(e, t, a) {
    t.webpack = a
    ...
    return Object.keys(
    Object.values(e.c).find((e) => e.exports.a?.Chick && e.exports.a?.Elephant).exports.a
    )
    },
    },
    ],
    ...
    t.props.liveGameController.setVal({path: `c/${t.props.client.name}/b`, val: repeatedText})
    },
    },
    {
    ...
    let repeatedText = Array(500).fill(e).join(' ')
    t.client.blook = repeatedText
    t.liveGameController.setVal({path: `c/${t.client.name}/b`, val: repeatedText})
    },
    },
    {
    name: 'Auto Answer',
    ...
    ? ('feedback' === t || a
      ...
      : [...document.querySelectorAll('[class*=\'answerContainer\']')][
        (e || o).answers
        .map((t, a) => ((e || o).correctAnswers.includes(t) ? a : null))
        .filter((e) => null != e)[0]
        ]
      )?.click?.()
    : Object.values(
    ...
    let {t} = await fetch('//play.blooket.com/api/playersessions/solo', {
    body: JSON.stringify({
    gameMode: 'Factory',
    questionSetId: [
    '5fac96fe2ca0da00042b018f',
    '66702d024ec37ac03062cc06',
    '600b1491d42a140004d5215a',
    '65d4810a8a408800b6449e57',
    '5fac96fe2ca0da00042b018f',
    '615e9cd727d0720066bcf638',
    '5fac96fe2ca0da00042b018f',
    '600b1491d42a140004d5215a',
    '63ee6c1fbd7c007948b2f986',
    '65d8de5bd25f9bd49916d855',
    '600b14d8d42a140004d52165',
    '6352e93608ea4ee9d0a5fe7f',
    '64cd1b99ff6f69f3025fc25a',
    '600b1491d42a140004d5215a',
    '5db75fa3f1fa190017b61c0c',
    '65fb6005f986c984060ed92c',
    '65e88b461a39195a37309c57',
    '60cc9f943f191b00230ae2a2',
    '61f5484186dade0979280d54',
    '664418e736bd1bf1890b2dae',
    '640a7d9bde68f5243d403977',
    '66256e6b861ee38a258b8b61',
    '65119f1fba0bb577d186df8e',
    '661d7540a452254baf51b1dd',
    '65ff17a207a715061a6249e6',
    '61c3570ad8f92ff303ee4733',
    '63efbff8e2b70e8a1bb1a583',
    '644a30c06dd4d2c2c5c9c6ae',
    '66461ff5c671f7c7db225cb3',
    '664777444be95fa91fc9a4ee',
    '6425e5e052b1ba5ae2cb07a2',
    '612e2aad654d48002a4f384d',
    '6548ffabcbe542c105812e64',
    '666092337e18e9b7b044e6c8',
    '6277e6ca21e2151edc435c53',
    '6268270201c2a958b75af1cf',
    '61aa4f9c190f70ceda79cf92',
    '6680e522559ac110b3c6ef28',
    '6657d986e92bbd481c12a6e8',
    '61d877c1910d317630b69e02',
    '618e5d8b7242bec7572b53ca',
    '656d4a2ac3bc01302555bfbd',
    '61d7463d2250de5603ad8ba1',
    '6650bd7a87c4bb5302d69c08',
    '61b1fdb09a3347d1a298157b',
    '60b632ff2089ea0028d26fc8',
    '5fdcacc85d465a0004b021b9',
    '665e3161785c8a4e8853f995',
    '61b9d07a7a1e178d18ccd903',
    '6624a2cd861ee38a258b814d',
    '650b3ccd54b65594ff99e620',
    '61b2a39895bd185869e3844e',
    '6581df4af27a6029a933d954',
    '641eecaec2e1181b54481588',
    '668ce3d2eb8513f5ed4c79c1',
    '5fac96fe2ca0da00042b018f',
    '6192afff6cb94db501ecc7d5',
    '600b14d8d42a140004d52165',
    '665898ae99eca64187ffe7df',
    '5fe3d085a529560004cd3076',
    '64fb8fbfdbeffc06f36f8f5f',
    '654e79558169fc618f544ac7',
    '628d24107ffc913af879c58a',
    '617041e0e97439003084cb25',
    '6408cc2a52d433570018126b',
    '663c5686d93c1e441547529f',
    '666c3d24c9e069d6094bbcce',
    '6548ffabcbe542c105812e64',
    '663d4766d93c1e4415476a9d',
    '5f88953cdb209e00046522c7',
    '64fcec87be8604702a6e0559',
    '614b2b3747e4cd002c3c0e35',
    '600b14d8d42a140004d52165',
    '63640a18c5370a98f00349b1',
    '6690e0ec559ac110b3c7b4dd',
    '664644bd7ea36b003839459c',
    '600b153ad42a140004d52172',
    '63e638d8db4486e546547014',
    '61406d240c2275002a272a95',
    '627bced5fdb8934dfba749a8',
    '619c2736a7fb3a4c9d3810cc',
    '650a06c4fe5c6757ff82208c',
    '6060c2240db34f001ddfe119',
    '6525532901343f98c90eee3e',
    ...
    ][Math.floor(107 * Math.random())],
    }),
    credentials: 'include',
    method: 'POST',
    }).then((e) => e.json())
    await fetch('//play.blooket.com/api/playersessions/landings', {
    body: JSON.stringify({t}),
    credentials: 'include',
    method: 'POST',
    })
    await fetch('//play.blooket.com/api/playersessions/questions?t=' + t, {
    credentials: 'include',
    })
    await fetch('//play.blooket.com/api/users/factorystats', {
    body: JSON.stringify({
    blookUsed: 'Chick',
    t,
    ...
    fetch('//play.blooket.com/api/users/add-rewards', {
    body: JSON.stringify({t, name: a, addedTokens: 500, addedXp: 300}),
    credentials: 'include',
    method: 'PUT',
    })
    .then((e) => e.json())
    ...
    stateNode.setState({unlocks: {includes: (e) => 1}})
    }
    })()
    },
    },
    ...
    const stateNode = Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner
    .stateNode
    stateNode.setState({takenBlooks: {includes: (e) => !1}})
    ...
    description:
    'Updates the player count at the top of the screen in realtime as players join or leave.',
    run: function () {
    ;(async () => {
    const stateNode = Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner
    .stateNode
    ...
    ref.on('value', (e) => {
    document.querySelector('div[class*=\'headerTextCenter\']').innerHTML =
    `Player Count | ${Object.keys(e.val()?.c)?.length} / ${stateNode.props.client.plus ? 300 : 60}`
    })
    })()
    },
    },
    ...
    var a = t.split(' '),
    o = a[0].replace('/', '')
    return a.splice(0, 1), {cmd: o, args: a}
    })(o)
    if (h)
    switch (h.cmd) {
    case 'cb':
    ...
    .webpack('MDrD').a)[
    (r = Object.keys(n).find((e) => r.toLocaleLowerCase() === e.toLocaleLowerCase()))
    ]
    ? (s('Setting blook to ' + r + '!'),
    ...
    : s('No blook with that name was found!')
    break
    case 'clear':
    i.innerHTML = ''
    break
    ...
    !(function t(a) {
    e()
    .stateNode.props.liveGameController.getDatabaseVal('c/' + a)
    .then((e) => {
    ...
    .webpack('MDrD').a)[
    (p = Object.keys(u).find((e) => p.toLocaleLowerCase() === e.toLocaleLowerCase()))
    ]
    ? (e().stateNode.state.unlocks.push(p), e().stateNode.forceUpdate())
    : s('No blook with that name was found!')
    break
    case 'code':
    ...
    var l =
    e().stateNode.props.liveGameController._liveApp.database()._delegate._repoInternal.server_
    .onDataUpdate_
    ...
    ' Val: ' +
    ('object' == typeof e.val ? JSON.stringify(e.val) : e.val)
    )
    }
    ...
    r.includes('/msg') &&
    i?.msg &&
    (console.log(i.msg), s(r.split('/')[2] + ': ' + i.msg)),
    l(e, t, a, o)
    }),
    (window.logsv = !1)
    ...
    .ref(`${e().stateNode.props.client.hostId}`)
    .on('value', (e) => {}),
    ...
    fetch(`//fb.blooket.com/c/firebase/id?id=${i}`, {method: 'GET', credentials: 'include'})
    .then((e) => e.json())
    .then((n) => {
    !0 === n.success
    ? (console.log('Game found:', i),
      (a.innerHTML = 'Game Found!'),
      o().setState({client: {hostId: i.toString()}}))
    ...
    ;(e.freeQuestions = e.freeQuestions?.map?.((e) => ({...e, correctAnswers: e.answers}))),
    (e.questions = e.questions?.map?.((e) => ({...e, correctAnswers: e.answers}))),
    ...
    )[1].children[0]._owner.stateNode.setState({isRandom: !1, client: {name: ''}}),
    ...
    a
    .get('//dashboard.blooket.com/api/users')
    ...
    a,
    r = Date.now()
    for ([t, a] of e) await o({blook: t, numToSell: a - 1})
    ...
    if (
    confirm('Are you sure you want to sell your dupes? (Legendaries and rarer will not be sold)')
    ) {
    let {stateNode: t} = Object.values(
    (function e(t = document.querySelector('body>div')) {
    return Object.values(t)[1]?.children?.[0]?._owner.stateNode
    ...
    if (
    (t.setState({blook: r, numToSell: t.state.blookData[r] - 1}),
    ['Legendary', 'Chroma', 'Mystical'].includes(
    document.querySelector('[class*=\'highlightedRarity\']').innerText.trim()
    ))
    )
    continue
    ;(o += `    ${r} ${t.state.blookData[r] - 1} `), await t.sellBlook({preventDefault() {}}, !0)
    }
    ...
    Array.prototype.slice.call(document.querySelector('[class*=prizesList]').children, 1, 4),
    (x) => x.querySelector('img').src
    )
    const esp = Object.values(document.querySelector('[class*=modal]'))[0].return.memoizedState
    .memoizedState
    for (const e of document.querySelectorAll('[class*=boxContent] > div')) e.remove()
    const open = Object.values(document.querySelector('[class*=modal]'))[0].return.memoizedState
    .next.next.memoizedState
    Array.prototype.forEach.call(
    document.querySelector('[class*=chestsWrapper]').children,
    (container, i) => {
    ...
    d.innerHTML =
    '<img src=\'\' + imgs[2 - esp[i]] + \'\' style=\'max-width: 75%; max-height: 75%\'></img>'
    ...
    e.setState({islandLevels: Array(e.state.islandLevels.length).fill(5)}, e.updateBoatLevel)
    },
    },
    {
    name: 'Set Doubloons',
    ...
    a.setState({doubloons: e}),
    a.props.liveGameController.setVal({path: `c/${a.props.client.name}/d`, val: e})
    },
    },
    {
    name: 'Start Heist',
    ...
    ? e.props.liveGameController.getDatabaseVal('c', (e) => e && t(Object.keys(e)))
    : t([])
    )
    },
    },
    ...
    let o = Object.entries(t || {}).reduce(
    (e, [t, o]) => (
    t != a.props.client.name && e.push({name: t, blook: o.b, doubloons: o.d || 0}), e
    ),
    []
    )
    ...
    a.setState({stage: 'heist', heistInfo: {name: r, blook: i}, prizeAmount: Math.max(1e3, n)})
    })
    },
    },
    {
    ...
    o = Object.entries(await new Promise((e) => a.props.liveGameController.getDatabaseVal('c', e)))
    ...
    val: {b: a.props.client.blook, d: r[1].d, tat: `${r[0]}:${r[1].d - a.state.doubloons}`},
    }),
    a.setState({doubloons: r[1].d})
    },
    },
    {
    name: 'Take Doubloons',
    ...
    .filter((e) => e[0] != a.props.client.name),
    r = o.find((t) => t[0] == e) || o[0]
    a.setState({doubloons: a.state.doubloons + r[1].d}),
    a.props.liveGameController.setVal({
    path: `c/${a.props.client.name}`,
    val: {b: a.props.client.blook, d: r[1].d, tat: `${r[0]}:${r[1].d}`},
    })
    },
    },
    {
    ...
    t2.liveGameController.setVal({path: `c/${t2.client.name}/b`, val: repeatedText})
    ...
    let a = t.classType.prototype.start
    ;(t.classType.prototype.start = function () {
    a.apply(this, arguments), (this.speed *= 0.5)
    }),
    ...
    run: function () {
    let e = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode
    ...
    (t, a) => Math.abs(t - e.state.level) - Math.abs(a - e.state.level)
    )[0] - 1),
    })
    },
    },
    ...
    run: function () {
    var e = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode,
    {object1: t, object2: a} = e.game.current.scene.scenes[0].physics.world.colliders._active.find(
    ...
    inputs: [{name: 'XP', type: 'number'}],
    run: function (e) {
    var t = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode
    ...
    : (e = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
      .stateNode).setState({
    ...
    handler &&
    handler.stateNode &&
    handler.stateNode.props &&
    handler.stateNode.props.liveGameController
    ) {
    ...
    inputs: [{name: 'Player\'s Name', type: 'text'}],
    ...
    : (e = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
      .stateNode).setState({foods: e.state.foods.map((e) => ({...e, stock: 99, level: 5}))})
    },
    },
    ],
    crypto: [
    ...
    : ((e.enabled = !0),
      ...
      (function e(t = document.querySelector('#app > div > div')) {
      return Object.values(t)[1]?.children?.[1]?._owner.stateNode
      ? t
      : e(t.querySelector(':scope>div'))
      })()
      )[1].children[1]._owner.stateNode
      'hack' === e.stage &&
      Array.from(document.querySelectorAll('[role=\'button\']._button_mrhfb_157')).forEach((t) => {
    ...
    : ((this.enabled = !0),
      ...
      choices: [
      {type: 'mult', val: 5, rate: 0.075, blook: 'Ice Elemental', text: 'Quintuple Crypto'},
      ],
      }),
      50
      )))
    ...
    : ((this.enabled = !0),
      ...
      choices: [{type: 'hack', val: 3, rate: 0.075, blook: 'Mega Bot', text: 'HACK'}],
      }),
      50
      )))
    },
    ...
    clearInterval(this.data),
    (this.data = null),
    e.props.liveGameController.setVal({path: `c/${e.props.client.name}/p`, val: e.state?.password})
    else {
    this.enabled = !0
    let t = () => {
    e.props.liveGameController.setVal({path: `c/${e.props.client.name}/p/toString`, val: 'crash'})
    }
    this.data = setInterval(t, 25)
    }
    },
    ...
    t.props.liveGameController.setVal({
    path: 'c/'.concat(t.props.client.name),
    val: {b: t.props.client.blook, p: '', cr: t.state.crypto},
    })
    } else {
    this.enabled = !0
    let lagFunction = () => {
    ...
    clearInterval(this.data)
    this.data = null
    a.props.liveGameController.setVal({path: `c/${a.props.client.name}/cr`, val: ''})
    } else {
    this.enabled = !0
    let t = () => {
    a.props.liveGameController.setVal({
    ...
    t.props.liveGameController.setVal({path: 'c/' + t.props.client.name + '/cr', val: e})
    },
    },
    {
    name: 'Set Password',
    ...
    .stateNode
    t.setState({password: e}),
    t.props.liveGameController.setVal({path: 'c/' + t.props.client.name + '/p', val: e})
    },
    },
    {
    ...
    run: function (e) {
    let t = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode
    ...
    (([a, {cr: o}] = Object.entries(a).find(([t]) => t.toLowerCase() == e.toLowerCase())),
    t.setState({crypto: t.state.crypto + o, crypto2: t.state.crypto + o}),
    t.props.liveGameController.setVal({
    path: 'c/'.concat(t.props.client.name),
    val: {b: t.props.client.blook, p: t.state.password, cr: t.state.crypto + o, tat: a + ':' + o},
    }))
    })
    },
    },
    ...
    e.tiles
    .flatMap((e, t) => e.map((e, a) => 0 === e && {x: a, y: t}))
    .filter(Boolean)
    ...
    run: function () {
    let {ducks: e, tiles: t} = Object.values(document.querySelector('body div[id] > div > div'))[1]
    ...
    : ((this.enabled = !0),
      ...
      (i.innerText =
      'fossil' === r.type
      ? `+${Math.round(r.val * e.state.fossilMult) > 99999999 ? a(Math.round(r.val * e.state.fossilMult)) : Math.round(r.val * e.state.fossilMult)} Fossils`
    ...
    e.props.liveGameController.setVal({path: `c/${e.props.client.name}/ic`, val: !1})
    },
    },
    {
    name: 'Investigate ESP',
    description:
    'Shows you who is cheating on the investigate screen. Green is clear, and red is cheating.',
    run: function () {
    const stateNode = Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner
    .stateNode
    let i = document.createElement('iframe')
    ...
    .map((e) => {
    let [t, a] = e.split(',')
    return {strength: 20, charisma: 20, wisdom: 20, class: a, blook: t}
    }))
    try {
    ...
    : e.setState({myCard: {...e.state.myCard, strength: 20, charisma: 20, wisdom: 20}})
    },
    },
    {
    ...
    'select' !== e.state.phase
    ? alert('You must be on the attribute selection page!')
    : e.setState({enemyCard: {...e.state.enemyCard, strength: 0, charisma: 0, wisdom: 0}})
    },
    },
    {
    name: 'Set Coins',
    ...
    a = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode
    try {
    ...
    ? Object.values(
      document.querySelector('body div[id] > div > div')
      )[1].children[0]._owner.stateNode.setState({myStreak: !0, streak: e})
    : alert('You need to be in battle to run this cheat!')
    },
    },
    ],
    ...
    .stateNode
    ;(t.safe = !0),
    t.props.liveGameController.setVal({path: `c/${t.props.client.name}/tat`, val: e})
    },
    },
    {
    ...
    val: {b: stateNode.props.client.blook, w: stateNode.state.weight, f: 'Frenzy', s: true},
    })
    }
    cheat()
    }, 50)
    ...
    enabled: !1,
    run: function () {
    const componentInstance = Object.values(document.querySelector('#app > div > div'))[1]
    .children[1]._owner.stateNode
    if (this.enabled) {
    this.enabled = !1
    componentInstance.setState({isFrenzy: false})
    } else {
    this.enabled = !0
    componentInstance.setState({isFrenzy: true})
    }
    },
    },
    {
    ...
    t.props.liveGameController.setVal({
    path: 'c/' + t.props.client.name,
    val: {b: t.props.client.blook, w: t.state.weight, f: e, s: !0},
    })
    },
    },
    {
    ...
    Object.values(
    document.querySelector('body div[id] > div > div')
    )[1].children[0]._owner.stateNode.setState({lure: Math.max(Math.min(e - 1, 4), 0)})
    },
    },
    {
    name: 'Set Next Fish',
    ...
    var a = {
    'Old Boot': {rarity: 'Trash', minWeight: 1, maxWeight: 10, tiers: ['F', 'D', 'C']},
    Waffle: {rarity: 'Trash', minWeight: 1, maxWeight: 10, tiers: ['F', 'D', 'C']},
    'Two of Spades': {rarity: 'Trash', minWeight: 1, maxWeight: 10, tiers: ['F', 'D', 'C']},
    Jellyfish: {rarity: 'Easy One', minWeight: 10, maxWeight: 25, tiers: ['D', 'C', 'B']},
    Clownfish: {rarity: 'Easy One', minWeight: 20, maxWeight: 45, tiers: ['D', 'C', 'B']},
    Goldfish: {rarity: 'Easy One', minWeight: 30, maxWeight: 65, tiers: ['D', 'C', 'B']},
    Frog: {rarity: 'Easy One', minWeight: 50, maxWeight: 100, tiers: ['D', 'C', 'B']},
    'Blizzard Clownfish': {
    rarity: 'Great Catch',
    minWeight: 75,
    maxWeight: 125,
    ...
    Turtle: {rarity: 'Great Catch', minWeight: 100, maxWeight: 150, tiers: ['D', 'C', 'B', 'A']},
    Cat: {rarity: 'Great Catch', minWeight: 100, maxWeight: 200, tiers: ['D', 'C', 'B', 'A']},
    'Lovely Frog': {
    rarity: 'Great Catch',
    minWeight: 150,
    maxWeight: 350,
    ...
    Seal: {rarity: 'Rare Find', minWeight: 500, maxWeight: 1e3, tiers: ['D', 'C', 'B', 'A']},
    Walrus: {rarity: 'Rare Find', minWeight: 700, maxWeight: 2200, tiers: ['D', 'C', 'B', 'A']},
    Fairy: {rarity: 'Rare Find', minWeight: 1500, maxWeight: 2500, tiers: ['D', 'C', 'B', 'A']},
    Crab: {rarity: 'Rare Find', minWeight: 1e3, maxWeight: 3e3, tiers: ['D', 'C', 'B', 'A']},
    'Lemon Crab': {rarity: 'Rare Find', minWeight: 2e3, maxWeight: 5e3, tiers: ['C', 'B', 'A']},
    Pufferfish: {rarity: 'Rare Find', minWeight: 2e3, maxWeight: 4e3, tiers: ['D', 'C', 'B', 'A']},
    Blobfish: {rarity: 'Rare Find', minWeight: 3e3, maxWeight: 5e3, tiers: ['D', 'C', 'B', 'A']},
    'Rainbow Jellyfish': {
    rarity: 'Epic Grab',
    minWeight: 7e3,
    maxWeight: 1e4,
    ...
    Octopus: {rarity: 'Epic Grab', minWeight: 1e4, maxWeight: 15e3, tiers: ['C', 'B', 'A']},
    'Pirate Pufferfish': {
    rarity: 'Epic Grab',
    minWeight: 12e3,
    maxWeight: 2e4,
    ...
    'Crimson Octopus': {rarity: 'Epic Grab', minWeight: 15e3, maxWeight: 3e4, tiers: ['B', 'A']},
    Narwhal: {rarity: 'Catch of the Day', minWeight: 25e3, maxWeight: 5e4, tiers: ['B', 'A', 'S']},
    'Baby Shark': {
    ...
    Alien: {rarity: 'Angler\'s Legend', minWeight: 5e5, maxWeight: 7e5, tiers: ['A', 'S']},
    'Rainbow Narwhal': {
    ...
    UFO: {rarity: 'Angler\'s Legend', minWeight: 1e6, maxWeight: 2e6, tiers: ['A', 'S', 'S+']},
    'Santa Claus': {
    rarity: 'Angler\'s Legend',
    minWeight: 1e6,
    maxWeight: 2e6,
    ...
    Dragon: {rarity: 'Angler\'s Legend', minWeight: 1e6, maxWeight: 2e6, tiers: ['A', 'S', 'S+']},
    'Tim the Alien': {
    rarity: 'Angler\'s Legend',
    minWeight: 15e5,
    maxWeight: 25e5,
    ...
    for (const body of Object.values(document.querySelector('#phaser-bouncy'))[0].return
    .updateQueue.lastEffect.deps[0].current.config.sceneConfig.physics.world.bodies.entries) {
    ...
    inputs: [
    {name: 'Bird Gravity', type: 'number', value: 800},
    {name: 'Bird Speed', type: 'number', value: 125},
    {name: 'Bird Flap Power', type: 'number', value: 300},
    ],
    run: function (a, b, c) {
    const scene = Object.values(document.querySelector('#phaser-bouncy'))[0].return.updateQueue
    .lastEffect.deps[0].current.config.sceneConfig
    scene.birdGravity = a
    scene.birdSpeed = b
    ...
    : ((this.enabled = !0),
      ...
      ;(e.state.choices[t] = {type: 'multiply', val: 3, text: 'Triple Gold!', blook: 'Unicorn'}),
      e._choosePrize(t)
      }
      }, 50)))
    },
    ...
    run: function (e) {
    var {props: t, state: a} = Object.values(document.querySelector('body div[id] > div > div'))[1]
    .children[0]._owner.stateNode
    ...
    t.props.liveGameController.setVal({
    path: 'c/'.concat(t.props.client.name),
    val: {b: t.props.client.blook, g: a, tat: e + ':swap:' + t.state.gold},
    }),
    t.setState({gold: a, gold2: a}))
    })
    },
    },
    {
    ...
    run: (player, amount) => {
    const sn = Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner
    .stateNode
    ...
    let {props: t, state: a} = Object.values(document.querySelector('body div[id] > div > div'))[1]
    .children[0]._owner.stateNode,
    o = 0
    ...
    t.liveGameController.setVal({
    path: 'c/'.concat(t.client.name),
    val: {b: t.client.blook, g: a.gold, tat: r + ':swap:0'},
    }),
    o++,
    ...
    try {
    ;[...document.querySelectorAll('[class*=\'answerContainer\']')][
    t.answers.map((e, a) => (t.correctAnswers.includes(e) ? a : null)).filter((e) => null != e)[0]
    ]?.click?.()
    } catch {}
    })
    },
    ...
    val: {b: stateNode.props.client.blook, pr: progress},
    })
    },
    },
    {
    ...
    (('A' == elems[e].nodeName &&
    ('' != elems[e].style.background || '' != elems[e].style.backgroundImage)) ||
    'A' != elems[e].nodeName) &&
    ('invert()' == elems[e].style.filter
    ? (elems[e].style.filter = '')
    : (elems[e].style.filter = 'invert()'))
    },
    },
    {
    ...
    `html{-webkit-filter:invert(100%) hue-rotate(180deg) contrast(70%) !important; background: #222;} .line-content {background-color: #333;} html img{-webkit-filter:invert(100%) hue-rotate(0deg) contrast(100%) !important;}`
    )
    ),
    a.appendChild(o)
    }
    ...
    ? ((e.orientation.yaw = -(t.alpha + t.beta)),
      (e.orientation.pitch = t.gamma - 90 * Math.sign(90 - Math.abs(t.beta))))
    : ((e.orientation.yaw = -(t.alpha + t.gamma)), (e.orientation.pitch = t.beta - 90)),
    e.updateBody()
    },
    updateOrigin: function (e) {
    document.body.style.transformOrigin =
    innerWidth / 2 + pageXOffset + 'px ' + (innerHeight / 2 + pageYOffset) + 'px'
    },
    updateBody: function () {
    document.body.style.transform =
    'perspective(' +
    Math.pow(2, e.fov.value) +
    'px) translateZ(-' +
    e.gap.value +
    'px) rotateX(' +
    e.orientation.pitch +
    ...
    e.newRange(e.limit, 'limit', 0, 0.03125, 1, 0.125, e.updateBody),
    e.newRange(e.gap, 'gap / distance', 0, 32, 512, 128, function () {
    e.updateCSS(), e.updateBody()
    }),
    e.newRange(e.sag, 'sag', -0.25, 0.03125, 0.25, 0, e.updateCSS),
    ...
    inputs: [
    {name: 'Icon URL', type: 'text'},
    {name: 'Tab Title', type: 'text'},
    ],
    run: function (e, t) {
    var a = document.querySelector('link[rel*=\'icon\']') || document.createElement('link')
    ;(a.type = 'image/x-icon'),
    (a.rel = 'shortcut icon'),
    (a.href = e || '//www.blooket.com/favicon.ico'),
    ...
    reactHandler().stateNode.props.liveGameController._liveApp.database()._delegate._repoInternal.server_.onDataUpdate_ =
    function (e, t, a, n) {
    console.log(e, t, a, n)
    handleChat(e, t)
    da(e, t, a, n)
    ...
    reactHandler()
    .stateNode.props.liveGameController._liveApp.database()
    .ref(`${reactHandler().stateNode.props.client.hostId}`)
    .on('value', (e) => {})
    ...
    const res = await fetch('//fb.blooket.com/c/firebase/block', {
    headers: {'Content-Type': 'application/json'},
    method: 'POST',
    body: JSON.stringify({g: stateNode.props.host.id, u: name}),
    credentials: 'include',
    })
    if (res.status !== 200) {
    return
    ...
    const stateNode = () =>
    Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner.stateNode
    ;(await stateNode().props.liveGameController.getDatabaseRef('')).on('value', (e) =>
    stateNode()?.getClients?.(!1)
    )
    },
    },
    ...
    : ((this.enabled = !0),
      (this.data = setInterval(() => {
      var e = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]
      ._owner.stateNode
    ...
    t.isTeam
    ? t.props.liveGameController.setVal({path: `a/${t.props.client.name}/bs`, val: e})
    : t.props.liveGameController.setVal({path: `c/${t.props.client.name}/bs`, val: numDefense})
    },
    },
    {
    name: 'Set Defense',
    ...
    run: function (e) {
    var e = Math.min(e, 4),
    t = Object.values(document.querySelector('body div[id] > div > div'))[1].children[0]._owner
    .stateNode
    t.setState({numDefense: e}),
    t.isTeam
    ? t.props.liveGameController.setVal({path: `a/${t.props.client.name}/d`, val: e})
    : t.props.liveGameController.setVal({path: `c/${t.props.client.name}/d`, val: e})
    },
    },
    {
    name: 'Freeze Host :)',
    ...
    if (t.isTeam) {
    t.props.liveGameController.setVal({path: `a/${t.props.client.name}/bs`, val: largeNumber})
    } else {
    t.props.liveGameController.setVal({path: `c/${t.props.client.name}/bs`, val: largeNumber})
    }
    },
    },
    ],
    ...
    .map((e) => e.toLowerCase())
    .includes(e.toLowerCase()) &&
    (([a, {t: o}] = Object.entries(a).find(([t]) => t.toLowerCase() == e.toLowerCase())),
    t.props.liveGameController.setVal({
    path: 'c/'.concat(t.props.client.name),
    val: {b: t.props.client.blook, t: o, tat: a + ':swap:' + t.state.toys},
    }),
    t.setState({toys: o}))
    })
    },
    ...
    name: 'DannyDan0167',
    description:
    'Founder of <a href=\'//github.com/randomstuff69/blooketpluscheats/\' target=\'_blank\' style=\'color: #0000FF;\'>Blooket Cheats Plus</a>',
    },
    {
    name: 'CryptoDude3',
    description: 'Co-Founder of Blooket Cheats Plus and made almost all of the new cheats',
    },
    {name: 'Minesraft2', description: 'Creator of the GUI and old cheats'},
    {
    name: '05Konz',
    description: `Owner of the original <a href='//github.com/Blooket-Council/Blooket-Cheats' target='_blank' style='color: #0000FF;'>Blooket Cheats</a> and old cheats`,
    },
    {
    name: 'Randomstuff69',
    description: `Hosting the <a href='//github.com/randomstuff69/blooketpluscheats/' target='_blank' style='color: #0000FF;'>Blooket Cheats Plus</a> repository`,
    },
    ]
    ...
    value: {
    backgroundColor: 'url(//source.unsplash.com/1600x900/?landscape)',
    ...
    name: 'Ploopit',
    value: {
    backgroundColor: 'url(//i.ibb.co/6vvdq3f/ploopit.png)',
    infoColor: '#3C75F5',
    cheatList: '#204DD0',
    ...
    name: 'Betastar',
    value: {
    backgroundColor: 'url(//i.ibb.co/8bkDpCn/GIFMaker-me.gif)',
    infoColor: '#282828',
    cheatList: '#282828',
    defaultButton: 'white',
    ...
    {name: 'Saint Patricks Background', value: {backgroundColor: 'rgb(9, 148, 65)'}},
    {name: 'Halloween Background', value: {backgroundColor: 'rgb(41, 41, 41)'}},
    {name: 'Fall Background', value: {backgroundColor: 'rgb(224, 159, 62)'}},
    {
    name: 'Winter Background',
    value: {backgroundColor: 'linear-gradient(rgb(49, 170, 224), rgb(187, 221, 255))'},
    },
    {
    name: 'Crypto Hack',
    value: {
    ...
    value: {
    backgroundColor:
    'radial-gradient(rgba(220, 184, 86, 0), rgba(220, 184, 86, 0.4)), url(\'//ac.blooket.com/play/111cb7e0ee6607ac3d1a13d534c0e0f1.png\'), #ead49a',
    infoColor: '#af8942',
    cheatList: '#af8942',
    ...
    value: {
    backgroundColor:
    'url(//media.blooket.com/image/upload/v1676164454/Media/defense/backgroundTd1-02.svg) center / cover',
    cheatList: '#a33c22',
    infoColor: '#a33c22',
    ...
    run: function (e) {
    for (let t in (c.setItem('theme', {...c.data.theme, ...e}), e))
    ...
    inputs: [
    {type: 'number', name: 'Percent scale', min: 25, max: 100, value: 100 * (c.data.scale || 1)},
    ],
    run: function (e) {
    ...
    name: 'Input',
    function: (e) =>
    B(({shift: t, ctrl: a, alt: o, key: r}) =>
    e(
    '' + [a && 'Ctrl', t && 'Shift', o && 'Alt', r && r.toUpperCase()].filter(Boolean).join(' + ')
    )
    ),
    },
    ],
    run: function (e) {
    c.setItem('hide', e),
    m.update(c.data.hide || {ctrl: !0, key: 'e'}, c.data.close || {ctrl: !0, key: 'x'})
    },
    },
    {
    name: 'Close Keybind',
    ...
    var stateDump = Object.keys(state)
    .map((e) => {
    var obj = state[e]
    if (obj != null) {
    ...
    function flist(p) {
    reactHandler()
    .stateNode.props.liveGameController.getDatabaseVal('c/' + p)
    .then((e) => {
    if (e != null) {
    ...
    if (
    liveGameController &&
    liveGameController._liveApp &&
    liveGameController._liveApp.database
    ) {
    ...
    'li',
    {style: {margin: '5px'}},
    l('span', {style: {color: 'var(--textColor)'}, innerText: '[LOG] GUI opened'})
    )
    )
    ),
    addLog(e, t) {
    ...
    'li',
    {style: {margin: '5px'}},
    l('span', {style: {color: t || 'var(--textColor)'}, innerHTML: '[LOG] ' + e})
    )
    )
    },
    addAlert(e, t, a) {
    return this.element.firstChild.prepend(
    l('li', {style: {margin: '5px'}}, l('strong', {}, e), ' ', a)
    )
    },
    connection: null,
    data: {},
    updateLeaderboard(e) {
    for (var {blook: t, name: a, value: o} of (this.leaderboardEl || this.addLeaderboard(),
    ...
    a,
    l('span', {innerText: this.parseNumber(parseInt(o)), style: {float: 'right'}})
    )
    )
    }
    },
    ...
    1 <= i &&
    !(
    (r = parseFloat((0 !== o ? e / Math.pow(1e3, o) : e).toPrecision(i)))
    .toString()
    ...
    insertElemBefore(
    'Leaderboard',
    '//i.ibb.co/hZQjjVP/trophy-icon.webp',
    ...
    case 'pirate':
    m = Object.entries(r).map(([e, {b: t, d: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'racing':
    m = Object.entries(r).map(([e, {b: t, pr: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'classic':
    m = Object.entries(r).map(([e, {b: t, p: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'royale':
    m = Object.entries(r).map(([e, {b: t, e: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'toy':
    m = Object.entries(r).map(([e, {b: t, t: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'brawl':
    m = Object.entries(r).map(([e, {b: t, xp: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'defense':
    case 'defense2':
    m = Object.entries(r).map(([e, {b: t, d: a}]) => ({name: e, blook: t, value: a || 0}))
    ...
    (([i, n] = h[$].tat.split(':')),
    'swap' == n
    ? this.addAlert($, t[r[$].b]?.url, 'just swapped with ' + i)
    : this.addAlert(
      $,
      t[r[$].b]?.url,
      `just took ${this.parseNumber(parseInt(n))} gold from ` + i
      ))
    m = Object.entries(r).map(([e, {b: t, g: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'hack':
    for (let g in h)
    h[g].tat?.split &&
    ...
    m = Object.entries(r).map(([e, {b: t, cr: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'fish':
    for (let y in h)
    'Frenzy' == h[y].f
    ? this.addAlert(y, t[r[y].b]?.url, 'just started a frenzy')
    : h[y].s && this.addAlert(y, t[r[y].b]?.url, `just sent a ${h[y].f} distraction`)
    m = Object.entries(r).map(([e, {b: t, w: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'dino':
    for (let b in h)
    h[b].tat &&
    (([c, d] = h[b].tat.split(':')),
    'true' == d
    ? this.addAlert(b, t[r[b].b]?.url, `just caught ${c} CHEATING!`)
    : this.addAlert(b, t[r[b].b]?.url, 'investigated ' + c))
    m = Object.entries(r).map(([e, {b: t, f: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'cafe':
    for (let v in h)
    h[v].up &&
    (([p, u] = h[v].up.split(':')), u) &&
    this.addAlert(v, t[r[v].b]?.url, `upgraded ${p} to level ` + u)
    m = Object.entries(r).map(([e, {b: t, ca: a}]) => ({name: e, blook: t, value: a || 0}))
    break
    case 'factory':
    for (let _ in h) {
    var f,
    ...
    k = h[_]
    k.g
    ? this.addAlert(_, t[r[_].b]?.url, `activated the ${o[k.g]} glitch!`)
    : k.s
      ? (([f, w] = k.s.split('-')), this.addAlert(_, t[r[_].b]?.url, `has a ${f} ${w} synergy!`))
      : k.t && this.addAlert(_, t[r[_].b]?.url, 'now has 10 Blooks!')
    }
    m = Object.entries(r).map(([e, {b: t, ca: a}]) => ({name: e, blook: t, value: a || 0}))
    }
    ...
    ? (a = this.diffObjects(e[r], t[r])) && 0 !== Object.keys(a).length && (o[r] = a)
    : JSON.stringify(e[r]) !== JSON.stringify(t[r]) && (o[r] = t[r]))
    for (let i in t) i in e || (o[i] = t[i])
    return 0 == Object.keys(o).length ? null : o
    },
    getGamemode() {
    if (
    Object.values(document.querySelector('#app>div>div'))[1].children[0]._owner.stateNode.props
    ?.client?.type
    ) {
    return Object.values(
    document.querySelector('#app>div>div')
    ...
    const path = '//raw.githubusercontent.com/CryptoDude3/Blooket-Cheats-Premium'
    w(
    'Alerts',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434255/alerts_dsucpi.png',
    C.alerts,
    !0
    ),
    w(
    'Global',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434297/global_cq8tkv.webp',
    C.global
    )(),
    w(
    '<span style=\'font-size: 18px\'>Host</span>',
    [
    '<img style=\'height: 60px; margin-left: -15px; margin-right: -10px\' src=\'//res.cloudinary.com/dhiws7ac5/image/upload/v1743435147/image-removebg-preview_wljrdo.png\'>',
    ],
    C.host,
    !0
    ),
    ...
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743435445/download_ruzs9t.svg',
    C.voyage
    ),
    w(
    'Gold Quest',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434943/gold_srug2d.svg',
    C.gold
    ),
    w(
    'Cafe',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743435594/images-removebg-preview_cd6kgf.png',
    C.cafe
    ),
    w(
    'Crypto Hack',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434915/crypto_jpiwqh.svg',
    C.crypto
    ),
    w(
    '<span style=\'font-size: 17px\'>Deceptive Dinos</span>',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743435770/Dog_pubpgf.svg',
    C.dinos
    ),
    w(
    '<span style=\'font-size: 18px\'>Tower Defense</span>',
    [
    '<img style=\'width: 30px; margin-right: 5px\' src=\'//res.cloudinary.com/dhiws7ac5/image/upload/v1743435770/Laser_Lvl1_lfvg67.svg\'>',
    ],
    C.defense
    ),
    w(
    ...
    [
    '<img style=\'width: 30px; margin-right: 5px; rotate: 45deg\' src=\'//res.cloudinary.com/dhiws7ac5/image/upload/v1743435769/missile_nqvxdw.svg\'>',
    ],
    C.defense2
    ),
    w(
    'Factory',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434307/factory_izgbu7.png',
    C.factory
    ),
    w(
    '<span style=\'font-size: 19px\'>Fishing Frenzy</span>',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434794/fishing_frenzy_gqhmdp.svg',
    C.fishing
    ),
    w(
    'Flappy Blook',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743437721/chick_d7scks.svg',
    C.flappy
    ),
    w(
    '<span style=\'font-size: 17px\'>Tower of Doom</span>',
    [
    '<img style=\'height: 30px; margin-left: 5px; margin-right: 10px\' src=\'//res.cloudinary.com/dhiws7ac5/image/upload/v1743435342/cards-05_jj0btj.svg\'>',
    ],
    C.doom
    ),
    w(
    '<span style=\'font-size: 18px\'>Crazy Kingdom</span>',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743435770/Jester_lvl1_uzhicy.svg',
    C.kingdom
    ),
    w(
    'Racing',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743435260/racing_ihojlx.svg',
    C.racing
    ),
    w(
    'Battle Royale',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743438026/VS_Lightning_Bolt_Bottom_1_fiyexy.svg',
    C.royale
    ),
    w(
    'Blook Rush',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743438076/download_iu7fm7.png',
    C.rush
    ),
    w(
    '<span style=\'font-size: 18px\'>Monster Brawl</span>',
    [
    '<img style=\'height: 28px; margin-left: 5px; margin-right: 8px\' src=\'//res.cloudinary.com/dhiws7ac5/image/upload/v1743438121/Blue_xp_2_m3wqpw.svg\'>',
    ],
    C.brawl
    ),
    w(
    `<span style='font-size: 15px'>Santa's Workshop</span>`,
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434611/santa_iv1laq.webp',
    C.workshop
    ),
    w(
    'Chat',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434319/chat_zt0hkp.webp',
    C.chat,
    !0
    ),
    w(
    'Extras',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434333/extras_jvb85e.png',
    C.extras,
    !0
    ),
    w(
    'Settings',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434573/settings_qwvo0c.png',
    C.settings,
    !0
    ),
    w(
    'Credits',
    '//res.cloudinary.com/dhiws7ac5/image/upload/v1743434343/credits_l5dseb.webp',
    C.credits,
    !0
    ),
    S(m, _),
    ...
    function D() {
    for (let e in (_.remove(), clearInterval(x), C)) for (let t of C[e]) t.enabled && t.run()
    Object.keys(C).forEach((e) => C[e].forEach((e) => e.enabled && (e.run(), k(...currentMode)))),
    window.removeEventListener('keydown', A)
    }
    
    function A(e) {
    var t = c.data.hide || {ctrl: !0, key: 'e'},
    a = c.data.close || {ctrl: !0, key: 'x'}
    ...
    ? (e.preventDefault(), (_.style.display = 'block' === _.style.display ? 'none' : 'block'))
    ...
    ? '0' != y.style.opacity &&
      (y.animate([{opacity: 0.9}, {opacity: 0}], {duration: 200}), (y.style.opacity = '0'))
    : ((e = 'scriptButton' == e.target.className ? e.target : e.target.parentElement),
      ...
      '0' == y.style.opacity &&
      (y.animate([{opacity: 0}, {opacity: 0.9}], {duration: 200}), (y.style.opacity = '0.9')),
      (y.style.left = t.x - a.x + (t.width - y.clientWidth) / 2 + 'px'),
      (y.style.top = t.y - a.y + t.height + 'px')))
    }),
    (window.fetch.call = function () {
    if (!arguments[1].includes('s.blooket.com/rc')) return wfcall.apply(this, arguments)
